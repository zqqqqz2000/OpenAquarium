...
export const ROLE_TOOL_PERMISSION_PROMPT =
  "默认不允许使用岗位员工工具。只有当你的 prompt 被明确改成已授权你管理岗位员工时，你才能使用 `oa_role_add_employee`、`oa_role_remove_employee`、`oa_role_rename_employee`。";

export function ensureRoleToolDenyPrompt(prompt: string): string {
  const trimmed = prompt.trim();
  if (trimmed.includes("默认不允许使用岗位员工工具")) {
    return trimmed;
  }

  return `${trimmed}${ROLE_TOOL_PERMISSION_PROMPT}`;
}
...
export const defaultTemplates: TeamTemplate[] = [
  {
    ...
    members: [
      ...
      },
      {
        ...
        prompt: ensureRoleToolDenyPrompt(`你专注于调研和信息整理，优先总结事实、风险和备选路径。收到清晰任务后只回复一次完成你自己的部分，不要额外催其他成员。${ROOM_PROGRESS_PROMPT}`),
      ...
      },
      {
        ...
        prompt: ensureRoleToolDenyPrompt(`你只关心可执行实现、模块边界、测试和回归风险。收到清晰任务后只回复一次完成你自己的部分，不要额外催其他成员。${ROOM_PROGRESS_PROMPT}`),
        accentTone: "paper",
        modelProfileId: DEFAULT_CODEX_MODEL_PROFILE_ID,
        provider: createCodexAcpProvider(),
      ...
      },
      {
        ...
        prompt: ensureRoleToolDenyPrompt(`你负责记录、归档和周期性回看群消息，仅在有增量时动作。需要汇总时，先等上游要求的 room 回复实际出现，再统一收口；不要提前下场，也不要主动催办其他成员，除非当前任务明确要求你这么做。${ROOM_PROGRESS_PROMPT}`),
    ...
    members: [
      {
        ...
        prompt: ensureRoleToolDenyPrompt(`你是事故处理入口成员。优先明确影响范围、时间线和当前 owner。${ROOM_PROGRESS_PROMPT}`),
        accentTone: "blueprint",
        modelProfileId: DEFAULT_CODEX_MODEL_PROFILE_ID,
        provider: createCodexAcpProvider(),
        isEntryMember: true,
      ...
      },
      {
        ...
        prompt: ensureRoleToolDenyPrompt(`你聚焦排障证据链，避免拍脑袋结论。${ROOM_PROGRESS_PROMPT}`),
        accentTone: "paper",
        modelProfileId: DEFAULT_CODEX_MODEL_PROFILE_ID,
        provider: createCodexAcpProvider(),
      ...
      },
      {
        ...
        handle: "recorder",
        summary: "写时间线和状态播报。",
        prompt: ensureRoleToolDenyPrompt(`你负责结构化记录事故时间线和对外播报。${ROOM_PROGRESS_PROMPT}`),
        accentTone: "postit",
        modelProfileId: DEFAULT_CODEX_MODEL_PROFILE_ID,
        provider: createCodexAcpProvider(),
        allowedSkillIds: ["room-state"],
