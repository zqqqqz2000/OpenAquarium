...
import { completeMemberTask, createProjectWithRoom, createWorkspaceSnapshot, runWatcher } from "../../domain/workspace";
  ...
  if (lead?.activeTaskId) {
    next = completeMemberTask(
      next,
      {
        ...
        finalContent: "先整理需求边界，然后 @builder 准备代码骨架，@research 收集现有 ACP 兼容层做法。",
      },
      context,
    );
  }
  ...
  if (researcher?.activeTaskId) {
    next = completeMemberTask(
      next,
      {
        ...
        finalContent: "现有 ACP 兼容层可以抽象成 provider descriptor，前端只依赖统一 schema。",
      },
      context,
    );
  }
  ...
  if (builder?.activeTaskId) {
    next = completeMemberTask(
      next,
      {
        ...
        finalContent: "ACP provider 我建议分成 `codex-acp` 和 `generic-acp` 两层，UI 只依赖统一 descriptor。",
      },
      context,
    );
  }
  ...
  if (scribe?.activeTaskId) {
    next = completeMemberTask(
      next,
      {
        ...
        finalContent: "已记录当前协作进展，后续 watcher 只会消费新增消息。",
      },
      context,
    );
  }
