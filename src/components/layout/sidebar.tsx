...
export function Sidebar(props: {
  ...
  return (
    <aside className="flex min-h-screen flex-col gap-5 px-4 py-5 md:px-5">
      <Card className="flex flex-col gap-4 p-5" tone="paper">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="m-0 text-sm text-[var(--muted-foreground)]">ACP multi-agent workspace.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {projects.map((project, projectIndex) => (
            <Card
              ...
              tone={project.id === activeProjectId ? "blueprint" : projectIndex % 2 === 0 ? "paper" : "postit"}
              style={project.id === activeProjectId ? tilt.active : tilt.positive}
            >
              <div>
                <p className="m-0 text-lg font-semibold tracking-tight">{project.name}</p>
                <p className="m-0 text-sm text-[var(--muted-foreground)]">{roomsByProject[project.id]?.length ?? 0} rooms</p>
              </div>
                ...
                {(roomsByProject[project.id] ?? []).map((room) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...wobbly.note,
                        background: room.id === activeRoomId ? "var(--secondary)" : "white",
                      }}
                    >
                      <span className="min-w-0">
                        <span className="block truncate text-sm font-medium">{room.name}</span>
                        <span className="block truncate text-xs text-[var(--muted-foreground)]">{summarizePrompt(room.topic, 40)}</span>
                      </span>
      ...
      <Card className="flex flex-col gap-3 p-4" tone="blueprint">
        ...
        <p className="m-0 text-sm text-[var(--muted-foreground)]">template 由首条问题绑定。后续可以新增 room，但不会在当前 room 中途替换成员阵容。</p>
      </Card>
    </aside>
  );
}
